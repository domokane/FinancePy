...
def test_valueCDSIndex():
    ...
    spd = cds_indexContract.par_spread(value_dt, issuer_curve, cds_recovery) * 10000.0
    test_cases.print("PAR SPREAD", spd)

    v = cds_indexContract.value(value_dt, issuer_curve, cds_recovery)
    test_cases.print("DIRTY VALUE", v["dirty_pv"])
    test_cases.print("CLEAN VALUE", v["clean_pv"])

    p = cds_indexContract.clean_price(value_dt, issuer_curve, cds_recovery)
    ...
    prot_pv = cds_indexContract.prot_leg_pv(value_dt, issuer_curve, cds_recovery)
    test_cases.print("PROTECTION LEG PV", prot_pv)

    prem_pv = cds_indexContract.premium_leg_pv(value_dt, issuer_curve, cds_recovery)

...
def main(start_index=0, end_index=None):
    ...
    for idx in range(start_index, end_index):
        ...
        module_golden_name = module_name

        if module_name.endswith("_2"):
            module_golden_name = module_name[:-2]  # remove the "_2" suffix
        else:
            module_golden_name = module_name
        ...
        try:
            ...
            golden_module = __import__(module_golden_name)

            num_errors = getattr(golden_module.test_cases, "_global_num_errors", 0)
            num_warnings = getattr(golden_module.test_cases, "_global_num_warnings", 0)
        ...
        except (FinError, value_error, name_error, type_error) as e:

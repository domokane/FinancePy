...
# global_test_case_mode = fin_test_case_mode.SAVE_TEST_CASES
global_test_case_mode = fin_test_case_mode.ANALYSE_TEST_CASES
# global_test_case_mode = fin_test_case_mode.DEBUG_TEST_CASES
...
########################################################################################
class fin_test_cases:
    """Test case framework for finance_py.
    ...
    fin_test_case_mode.SAVE_TEST_CASES.
    - The second step is that we change the value of global_test_case_mode to
    fin_test_case_mode.ANALYSE_TEST_CASES and then run the test scripts. This time
    ...
    def __init__(self, module_name, mode):
        """Create the test_case given the module name and whether we are in
        ...
        if mode in fin_test_case_mode:
            self._mode = mode
        else:
            raise FinError("Unknown test_case Mode")

        if mode == fin_test_case_mode.DEBUG_TEST_CASES:
            # Don't do anything
            self._verbose = True
            return
        ...
        # When we do pep8 conversion we add a _2 to the module name but
        # we don't want to do that for the test cases to use golden files
        if self._module_name.endswith("_2"):
            self._module_name = self._module_name[:-2]
        ...
        self._golden_file_name = join(
            self._golden_folder, self._module_name + "_GOLDEN.test_log"
        )

        self._compare_file_name = join(
            self._compare_folder, self._module_name + "_COMPARE.test_log"
        )

        self._differences_file_name = join(
            self._differences_folder, self._module_name + "_DIFFS.test_log"
        )

        if self._mode == fin_test_case_mode.SAVE_TEST_CASES:
    ...
    def print(self, *args):
        ...
        if self._mode == fin_test_case_mode.DEBUG_TEST_CASES:
            print(args)
            return
    ...
    def compare_test_cases(self):
        ...
        #        max_num_lines = max(num_golden_lines, num_compare_lines)
        ...
        #            for row_num in range(min_num_lines,max_num_lines):

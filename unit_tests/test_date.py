...
def test_Date():
    ...
    next_cds_date = start_dt.next_cds_date(0)
    assert next_cds_date == Date(20, 3, 2018)

    # Checking for num_months = 120
    next_cds_date = start_dt.next_cds_date(120)
    assert next_cds_date == Date(20, 3, 2028)

    start_dt = Date(1, 1, 2018)

    # Checking for num_days = 1
    next_imm_date = start_dt.add_days(1).next_imm_date()
...
def test_DateTenors():
    ...
    tenor = "5d"
    assert start_dt.add_tenor(tenor) == Date(28, 2, 2018)

    tenor = "7D"
    assert start_dt.add_tenor(tenor) == Date(2, 3, 2018)

    tenor = "1W"
    assert start_dt.add_tenor(tenor) == Date(2, 3, 2018)

    tenor = "4W"
    assert start_dt.add_tenor(tenor) == Date(23, 3, 2018)

    tenor = "1M"
    assert start_dt.add_tenor(tenor) == Date(23, 3, 2018)

    tenor = "24M"
    assert start_dt.add_tenor(tenor) == Date(23, 2, 2020)

    tenor = "2Y"
    assert start_dt.add_tenor(tenor) == Date(23, 2, 2020)

    tenor = "10y"
    assert start_dt.add_tenor(tenor) == Date(23, 2, 2028)

    tenor = "0m"
    assert start_dt.add_tenor(tenor) == Date(23, 2, 2018)

    tenor = "20Y"
    assert start_dt.add_tenor(tenor) == Date(23, 2, 2038)
...
def test_DateAddYears():
    ...
    years = [1, 3, 5, 7, 10]
    dates_list = start_dt.add_years(years)
    ...
    dates_numpy = start_dt.add_years(years)
    ...
    dates_fractional = start_dt.add_years(years)
    ...
    dates_fractional_np = start_dt.add_years(years)

...
def test_oisfr_as_only():
    ...
    libor_curve = OISCurve(settle_dt, fras, swaps)

    print("DATE", "MATDATE", "VALUE")
...
def test_ois_deposits_fr_as_swaps():

    value_dt = Date(18, 9, 2019)

    dcc_type = DayCountTypes.THIRTY_E_360_ISDA
    depos = []
    ...
    fra = IborFRA(fra_settle_dt, fra_maturity_dt, fra_rate, dcc_type)
    fras.append(fra)
    ...
    libor_curve = OISCurve(value_dt, depos, fras, swaps)

    df = libor_curve.df(settle_dt)

    print("SETTLEMENT DATE", "DF")
    print(str(settle_dt), df)
    print("DATE", "DF")

    for deposit in depos:
        df = libor_curve.df(deposit.maturity_dt)
        print(str(deposit.maturity_dt), df)

    for swap in swaps:
        df = libor_curve.df(swap.maturity_dt)
        print(str(swap.maturity_dt), df)
...
def future_tofra_rate(price, convexity):
...
def test_ois_deposits_futures_swaps():
    ...
    depo = IborDeposit(settle_dt, "1D", 1.712 / 100.0, depo_dcc_type)
    depos = [depo]
    ...
    fra = IborFRA(fra_settle_dt, fra_maturity_dt, fra_rate, depo_dcc_type)
    fras.append(fra)
...
def test_derivative_pricing_example():
    ...
    swap_rate = 0.0058
    swap = OIS(settle_dt, "1Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)

    swap_rate = 0.0060
    swap = OIS(settle_dt, "2Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)

    swap_rate = 0.0072
    swap = OIS(settle_dt, "3Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)

    swap_rate = 0.0096
    swap = OIS(settle_dt, "4Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)

    swap_rate = 0.0124
    swap = OIS(settle_dt, "5Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)

    swap_rate = 0.0173
    swap = OIS(settle_dt, "7Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)

    swap_rate = 0.0219
    swap = OIS(settle_dt, "10Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)

    swap_rate = 0.0283
    swap = OIS(settle_dt, "30Y", fixed_leg_type, swap_rate, freq_type, dc_type)
    swaps.append(swap)
    ...
    for _ in range(0, num_repeats):
        _ = OISCurve(value_dt, fras, swaps, InterpTypes.FLAT_FWD_RATES)
    ...
    print("METHOD", "TIME")
    print("NON-LINEAR SOLVER BOOTSTRAP", elapsed1 / num_repeats)
    print("LINEAR SWAP BOOTSTRAP", elapsed2 / num_repeats)
...
# test_derivative_pricing_example()
# test_oisfr_as_only()
# test_ois_deposits_fr_as_swaps()
# test_ois_deposits_futures_swaps()
...
swaps = []
swap = OIS(
    settle_dt, "2Y", pay_rec, (2.77417 + 2.77844) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "3Y", pay_rec, (2.86098 + 2.86582) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "4Y", pay_rec, (2.90240 + 2.90620) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "5Y", pay_rec, (2.92944 + 2.92906) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "6Y", pay_rec, (2.94001 + 2.94499) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "7Y", pay_rec, (2.95352 + 2.95998) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "8Y", pay_rec, (2.96830 + 2.97400) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "9Y", pay_rec, (2.98403 + 2.98817) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "10Y", pay_rec, (2.99716 + 3.00394) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "11Y", pay_rec, (3.01344 + 3.01596) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "12Y", pay_rec, (3.02276 + 3.02684) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "15Y", pay_rec, (3.04092 + 3.04508) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "20Y", pay_rec, (3.04417 + 3.05183) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "25Y", pay_rec, (3.03219 + 3.03621) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "30Y", pay_rec, (3.01030 + 3.01370) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "40Y", pay_rec, (2.96946 + 2.97354) / 200, freq, accrual
)
swaps.append(swap)
swap = OIS(
    settle_dt, "50Y", pay_rec, (2.91552 + 2.93748) / 200, freq, accrual
)
swaps.append(swap)
...
print("VALUATION TO TODAY DATE", " PV")
print("VALUE:", swaps[0].value(value_dt, ois_curve, None))
print("FIXED:", -swaps[0].fixed_leg.value(value_dt, ois_curve))
print(
    "FLOAT:", swaps[0].float_leg.value(value_dt, ois_curve, None)
)

print("VALUATION TO SWAP SETTLEMENT DATE", " PV")
print("VALUE:", swaps[0].value(settle_dt, ois_curve, None))
print("FIXED:", -swaps[0].fixed_leg.value(settle_dt, ois_curve))
print(
    "FLOAT:", swaps[0].float_leg.value(settle_dt, ois_curve, None)
)

########################################################################################
# Copyright (C) 2018, 2019, 2020 Dominic O'Kane
########################################################################################

import glob
from os.path import dirname, basename, join
import traceback
import time
import runpy


import financepy

print("FinancePy imported successfully from:", financepy.__file__)

from financepy.utils.date_format import set_date_format, DateFormatTypes
from financepy.utils.error import FinError

# ============================================================================
# FINANCEPY EXAMPLES - Run All Tests
# ============================================================================

# This only works if I have an init.py in the parent folder
set_date_format(DateFormatTypes.UK_LONG)

###############################################################################


def main(start_index=0, end_index=None):
    """Loop over test cases"""

    # I put this here to get the library loaded and header printed before loop
    test_folder = dirname(__file__)
    print("Looking in folder:", test_folder)
    modules = sorted(glob.glob(join(test_folder, "**", "test_*.py"), recursive=True))
    num_modules = len(modules)

    if end_index is None or end_index > num_modules:
        end_index = num_modules

    timings = []

    # start_index = 40
    #    end_index = 115

    for idx in range(start_index, end_index):

        module_path = modules[idx]
        module_name = basename(module_path)[:-3]

        print(
            f"TEST: {idx + 1:3d} of {num_modules:3d}: {module_name:<35} ",
            end="",
        )

        start_time = time.perf_counter()

        try:
            namespace = runpy.run_path(module_path)
            test_cases = namespace.get("test_cases")
            num_errors = getattr(test_cases, "_global_num_errors", 0)
            num_warnings = getattr(test_cases, "_global_num_warnings", 0)

            elapsed = time.perf_counter() - start_time
            timings.append((module_name, elapsed))

            # print(f"WARNINGS: {num_warnings:3d} ERRORS: {num_errors:3d} ", end="")
            print(
                f"TIME: {elapsed:6.3f} s " f"WARNINGS: {num_warnings:3d} ERRORS: {num_errors:3d}",
                end="",
            )

            if num_errors > 0:
                print("*" * 1, end="")

            print()

        except (FinError, ValueError, NameError, TypeError) as e:
            elapsed = time.perf_counter() - start_time
            timings.append((module_name, elapsed))
            print(f"{type(e).__name__}: {e} ************ (TIME: {elapsed:6.3f} s)")
        except Exception as e:
            elapsed = time.perf_counter() - start_time
            timings.append((module_name, elapsed))
            print(f"Unexpected {type(e).__name__}: {e} (TIME: {elapsed:6.3f} s)")
            traceback.print_exc()


if __name__ == "__main__":
    # Optionally customize start and end test indices here
    main()

###############################################################################

...
def test__hull_book_examples(capsys):
    ...
    num_factors = 1
    gammas1_f_list = [0.00, 0.1550, 0.2063674, 0.1720986, 0.1721993, 0.1524579,
                    0.1414779, 0.1297711, 0.1381053, 0.135955, 0.1339842]
    gammas1_f = np.array(gammas1_f_list)

    # One factor model
    fwds1_f = lmm_simulate_fwds_1f(num_fwds, num_paths, numeraire_index, fwd0,
                                  gammas1_f, taus, use_sobol, seed)

########################################################################################

#    LMMPrintForwards(fwds1_f)

    v_ratchet_caplets = lmm_ratchet_caplet_pricer(spread, num_fwds, num_paths,
                                                fwd0, fwds1_f, taus) * 100.0

    hull_ratchet_caplets1_f = [0.00, 0.196, 0.207, 0.201, 0.194, 0.187,
    ...
    v_sticky_caplets = lmm_sticky_caplet_pricer(spread, num_fwds, num_paths,
                                              fwd0, fwds1_f, taus) * 100.0

    hull_sticky_caplets1_f = [0.0, 0.196, 0.336, 0.412, 0.458, 0.484,
                           0.498, 0.502, 0.501, 0.497, 0.488]

    check_vector_differences(v_sticky_caplets, hull_sticky_caplets1_f, 1e-2)

    num_factors = 1
    lambdas1_f_list = [[0.0, 0.1550, 0.2064, 0.1721, 0.1722, 0.1525,
                      0.1415, 0.1298, 0.1381, 0.1360, 0.1340]]
    lambdas1_f = np.array(lambdas1_f_list)

    # One factor model
    fwds_mf = lmm_simulate_fwds_mf(num_fwds, num_factors, num_paths, numeraire_index,
                                  fwd0, lambdas1_f, taus, use_sobol, seed)

    v_ratchet_caplets = lmm_ratchet_caplet_pricer(spread, num_fwds, num_paths,
                                                fwd0, fwds_mf, taus) * 100.0

    hull_ratchet_caplets1_f = [0.0, 0.196, 0.207, 0.201, 0.194, 0.187,
                            0.1890, 0.172, 0.167, 0.160, 0.153]
    ...
    hull_sticky_caplets1_f = [0.00, 0.196, 0.336, 0.412, 0.458, 0.484, 0.498,
                           0.502, 0.501, 0.497, 0.488]

    check_vector_differences(v_sticky_caplets, hull_sticky_caplets1_f, 1e-2)

    num_factors = 3
    lambdas3_f_list = [[0.00, 0.1365, 0.1928, 0.1672, 0.1698, 0.1485,
    ...
    # Three factor model
    fwds3_f = lmm_simulate_fwds_mf(num_fwds, num_factors, num_paths, numeraire_index,
                                  fwd0, lambdas3_f, taus, use_sobol, seed)

    hull_ratchet_caplets3_f = [0.00, 0.194, 0.207, 0.205, 0.198, 0.193,
                            0.189, 0.180, 0.174, 0.168, 0.162]

    v_ratchet_caplets = lmm_ratchet_caplet_pricer(spread, num_fwds, num_paths,
                                                fwd0, fwds3_f, taus) * 100.0
    ...
    hull_sticky_caplets3_f = [0.00, 0.195, 0.336, 0.418, 0.472, 0.506,

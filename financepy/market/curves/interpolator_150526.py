...
class Interpolator:
    ...
    def fit(self, times: np.ndarray, dfs: np.ndarray):
        ...
        # Not enough data to do a fit so abandon until we have more
        if len(times) == 1:
            return

        if self._interp_type == InterpTypes.PCHIP_LOG_DISCOUNT:
            self._interp_fn = PchipInterpolator(self._times, np.log(self._dfs))

        elif self._interp_type == InterpTypes.PCHIP_ZERO_RATES:
            zero_rates = -np.log(self._dfs) / (self._times + G_SMALL)
            ...
            self._interp_fn = CubicSpline(
                self._times, np.log(self._dfs), bc_type="natural"
            )
    ...
    def _interpolate_linear_on_fwd(self, tvec: np.ndarray) -> np.ndarray:
        """Special handling for LINEAR_ONFWD_RATES interpolation."""
        if self._interp_fn is None:
            # Fallback during curve construction
            log_dfs = np.log(self._dfs)
            return np.exp(np.interp(tvec, self._times, log_dfs))

        # Normal case with fitted spline
        last_t = self._times[-1]
        result = np.zeros_like(tvec)

        for i, t in enumerate(tvec):
            if t <= 0.0:
                result[i] = 1.0
            elif t <= last_t:
                result[i] = np.exp(-self._interp_fn.integral(0.0, t))
            else:
                # Constant forward rate extrapolation
                integral = self._interp_fn.integral(0.0, last_t)
                last_fwd = -np.log(self._dfs[-1]) / last_t
                result[i] = np.exp(-(integral + last_fwd * (t - last_t)))
        return result
    ...
    def interpolate(
        ...
        """Interpolation of discount factors at time(s) t.

        Returns scalar if input is scalar, array if input is array.
        """
        if self._dfs is None:
            raise FinError("Discount factors have not been set.")

        # === Input handling ===
        is_scalar = isinstance(t, (float, np.float64))
        tvec = np.asarray(t, dtype=float).ravel()

        if np.any(tvec < 0.0):
            raise FinError("Interpolate times must all be >= 0")

        # Very small t -> DF = 1.0
        if len(tvec) == 1 and abs(tvec[0]) < G_SMALL:
            return 1.0

        # ==================== FALLBACK: When spline not yet fitted ====================
        if self._interp_fn is None:
            # Safe linear interpolation on log(DF) during bootstrapping
            log_dfs = np.log(self._dfs)
            out = np.exp(np.interp(tvec, self._times, log_dfs))
            return float(out[0]) if is_scalar else out

        # ==================== Normal interpolation after fitting ====================
        if self._interp_type in [
            ...
            out = self._interpolate_linear_on_fwd(tvec)
        ...
        # Return correct type
        if is_scalar:
            return float(out[0]) if isinstance(out, np.ndarray) else float(out)
        return out.reshape(np.shape(t))

    ###########################################################################

    def interpolate_old(self, t: float):
        ...
        is_scalar = isinstance(t, (float, np.float64))
        input_shape = np.shape(t)

        tvec = np.asarray(t, dtype=float).ravel()

        if np.any(tvec < 0.0):
            raise FinError("Interpolation times must all be >= 0")
        ...
        if self._interp_fn is None:
            ...
            log_df_interp = np.interp(
                tvec, self._times, log_dfs, left=0.0, right=None
            )
            out = np.exp(log_df_interp)
            return out
        ...
        # We always return a numpy 1D array even if length is one

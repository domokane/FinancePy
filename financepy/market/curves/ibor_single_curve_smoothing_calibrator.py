...
class IborSingleCurveSmoothingCalibrator(object):
    ...
    def _repricing_objectives(self, curve_to_use=None):
        ...
        for depo in curve.used_deposits:
            ...
            # as rate
            r = -np.log(depo.value(value_dt, curve) / depo.notional) / acc_factor
            out[idx] = r
            idx = idx + 1
        ...
        for fra in curve.used_fras:
            ...
            v = fra.value(value_dt, curve) / fra.notional / acc_factor
            out[idx] = v
            idx = idx + 1

        for swap in curve.used_swaps:
            v = swap.value(value_dt, curve) / swap.fixed_leg.notional / swap.pv01(value_dt, curve)
            out[idx] = v
            idx = idx + 1

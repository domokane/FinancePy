...
def test_FinBasketOption():
    ...
    for beta in betaList:    
        for numPaths in [10000]:    
            callOption = FinBasketOption(expiryDate, 100.0, FinOptionTypes.EUROPEAN_CALL,numAssets)
            betas = np.ones(numAssets) * beta
            start = time.time()
            v = callOption.value(valueDate,stockPrices,interestRate,dividendYields,volatilities, betas)
            vMC = callOption.valueMC(valueDate,stockPrices,interestRate,dividendYields,volatilities, betas, numPaths)
            end = time.time()
            duration = end-start
    ...
    for beta in betaList:    
        for numPaths in [10000]:    
            callOption = FinBasketOption(expiryDate, 100.0, FinOptionTypes.EUROPEAN_PUT,numAssets)
            betas = np.ones(numAssets) * beta
            start = time.time()
            v = callOption.value(valueDate,stockPrices,interestRate,dividendYields,volatilities, betas)
            vMC = callOption.valueMC(valueDate,stockPrices,interestRate,dividendYields,volatilities, betas, numPaths)

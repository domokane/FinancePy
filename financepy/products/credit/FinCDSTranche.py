    ...
    def valueBC(self, 
        ...
        for i in range(1,numTimes):
           ...
           for j in range(0,numCredits):
                ...
                qVector[j] = interpolate(t, vTimes, qRow, FinInterpMethods.FLAT_FORWARDS.value)

           if model == FinLossDistributionBuilder.RECURSION:
               qt1[i] = trSurvProbRecursion(0.0, k1, numCredits, qVector, recoveryRates, betaVector1, numPoints)
               qt2[i] = trSurvProbRecursion(0.0, k2, numCredits, qVector, recoveryRates, betaVector2, numPoints)        
           elif model == FinLossDistributionBuilder.ADJUSTED_BINOMIAL:        
               qt1[i] = trSurvProbAdjBinomial(0.0, k1, numCredits, qVector, recoveryRates, betaVector1, numPoints)
               qt2[i] = trSurvProbAdjBinomial(0.0, k2, numCredits, qVector, recoveryRates, betaVector2, numPoints)        
           elif model == FinLossDistributionBuilder.GAUSSIAN:        
               qt1[i] = trSurvProbGaussian(0.0, k1, numCredits, qVector, recoveryRates, betaVector1, numPoints)
               qt2[i] = trSurvProbGaussian(0.0, k2, numCredits, qVector, recoveryRates, betaVector2, numPoints)        
           elif model == FinLossDistributionBuilder.LHP:        
               qt1[i] = trSurvProbLHP(0.0, k1, numCredits, qVector, recoveryRates, beta1)
               qt2[i] = trSurvProbLHP(0.0, k2, numCredits, qVector, recoveryRates, beta2)        
           else:

...
def par_rate_risk_report(base_curve: IborSingleCurve,  trades: list, trade_labels: list = None, bump_size=1.0*gBasisPoint):
    ...
    risk_report = benchmarks_report[[
        'type', 'start_date', 'maturity_date', 'market_rate']].copy()
    ...
    for trade, trade_label in zip(trades, trade_labels):
        base_values[trade_label] = trade.value(
            base_curve.value_dt, base_curve)
    ...
    for benchmark_idx in range(n_benchmarks):
        ...
        for trade_idx, trade in enumerate(trades):
            ...
            bumped_value = trade.value(
                bumped_curve.value_dt, bumped_curve)
...
def forward_rate_risk_report(base_curve: DiscountCurve,
    ...
    """Generate forward rate deltas (forward delta ladder) risk report, which is the sensitivity of trades to bucketed
    ...
    grid, grid_labels = _grid_from_dates_tenor(grid_last_date, grid_bucket_tenor, valuation_date)
...
def forward_rate_risk_report_custom_grid(
    ...
    if grid_labels is None:
        ...
        tenor_in_days = [datediff(s, m) for s, m in zip(risk_report['start_date'], risk_report['maturity_date'])]
    ...
    for trade, trade_label in zip(trades, trade_labels):
        val_res = trade.value(
            asof, base_curve, pv_only=False)
        base_values[trade_label] = val_res[0]

        cf_report = val_res[1]
        cf_report_first_period = cf_report[(cf_report['payment_date'] > grid[0]) &
    ...
    for fwdrate_idx in range(len(risk_report)):
        ...
        base_rate = base_curve.fwd_rate(start_date, maturity_date, DayCountTypes.SIMPLE)
...
def carry_rolldown_report(base_curve: DiscountCurve,
                          grid_last_date: Date,
                          grid_bucket_tenor: Union[str, Tenor],
                          trades: list,  trade_labels: list = None, bump_size=1.0*gBasisPoint,):
    """Generate carry and rolldown risk report based on the sensitivity of trades to bucketed
    shocks of the instantaneous (ON) forward rates. Here shock_i is applied to the ON forward rates
    over [t_i, t_{i+1}] where {t_i} is a time grid from base_curve.valuation_date to grid_last_date with
    buckets of size grid_bucket_tenor
    ...
    Returns:
        (dict, Dataframe): (base_values, risk_report) 
            base_value is a dictionary with trade_labels as keys and base trade values as values
            risk_report is a dataframe with time buickets for rows and carry/rolldown and DV01 columns
            per trade, and a total for all trades for each measure. Columns marked 'ROLL' have carry in the 
    ...
    for label in trade_labels:
        ...
        risk_report.loc[1:, ROLL_PREFIX+label] = risk_report.loc[1:, DV01_PREFIX+label] * rate_change[1:] / gBasisPoint

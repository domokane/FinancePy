...
class BondYieldCurve:
    ...
    def __init__(
        self, settle_dt: Date, bonds: list, ylds: np.ndarray | list, curve_fitter
    ):
        ...
        if fit_type is CurveFitPolynomial:
            ...
            (popt, pcov) = curve_fit(
                curve_fitter.interp_yield, xdata, ydata, bounds=curve_fitter.bounds
            )
            ...
            spline = splrep(xdata, ydata, k=curve_fitter.power, t=curve_fitter.knots)
    ...
    def interp_yield(self, maturity_dt: Date):
        "Interpolate the yield according to the model type"

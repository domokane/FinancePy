...
def f(s0, *args):
    ...
    objFn = self.value(valueDate,s0,interestRate,dividendYield,modelType,modelParams) - value    
...
class FinCompoundOption(FinOption):
    ...
    def value(self,
        ...
        sstar = self.impliedStockPrice(s0, self._expiryDate1, self._expiryDate2,
                                       k1, k2, self._optionType2, r, q, v)
        ...
        if self._optionType1 == CALL and self._optionType2 == CALL:
            v = s0*dq2*phi2(a1,b1,c) - k2*df2*phi2(a2,b2,c) - df1*k1*N(a2)
        elif self._optionType1 == PUT and self._optionType2 == CALL:
            v = k2*df2*phi2(-a2,b2,-c) - s0*dq2*phi2(-a1,b1,-c) + df1*k1*N(-a2)
...
###############################################################################
    ...
    def impliedStockPrice(self,
        ...
        argtuple = (option,expiryDate1,interestRate,dividendYield,volatility,strikePrice1)
        sigma = optimize.newton(f, x0=stockPrice, args=argtuple, tol=1e-8, maxiter=50, fprime2=None)
        return sigma

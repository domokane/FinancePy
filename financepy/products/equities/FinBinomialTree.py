...
class FinTreeExerciseTypes(Enum):
    EUROPEAN=1
    AMERICAN=2
...
def validatePayoff(payoffType,payoffParams):
    ...
    if payoffType == FinTreePayoffTypes.FWD_CONTRACT.value:
        numParams = 1
    elif payoffType == FinTreePayoffTypes.VANILLA_OPTION.value:
        numParams = 2
    elif payoffType == FinTreePayoffTypes.DIGITAL_OPTION.value:
        numParams = 2
    elif payoffType == FinTreePayoffTypes.POWER_CONTRACT.value:
        numParams = 2
    elif payoffType == FinTreePayoffTypes.POWER_OPTION.value:
        numParams = 3
    elif payoffType == FinTreePayoffTypes.LOG_CONTRACT.value:
        numParams = 0
    elif payoffType == FinTreePayoffTypes.LOG_OPTION.value:
        numParams = 1
    else: 
        raise FinError("Unknown payoff type")

    if len(payoffParams) != numParams:
        raise FinError("Number of parameters required for "+ str(payoffType) + " must be " + str(numParams))
...
#        validatePayoff(payoffType.value,payoffParams)
        ...
        for iNode in range(0, iTime+1):    
            ...
            optionValues[index + iNode] = payoffValue(s,payoffTypeValue,payoffParams)
        ...
        for iTime in range(numSteps-1,-1,-1):
            ...
            for iNode in range(0,iTime+1):
                ...
                if exerciseType == FinTreeExerciseTypes.EUROPEAN:
                    optionValues[index + iNode] = holdValue
                elif exerciseType == FinTreeExerciseTypes.AMERICAN:
                    s = stockValues[index + iNode]

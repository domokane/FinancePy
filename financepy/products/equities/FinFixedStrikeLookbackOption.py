...
class FinFixedStrikeLookbackOptionTypes(Enum):
    FIXED_CALL=1
    FIXED_PUT=2
...
################################################################################
    ...
    def value(self,
        ...
        if self._optionType == FinFixedStrikeLookbackOptionTypes.FIXED_CALL:
            smax = stockMinMax
            if smax < s0:
                raise FinError("The Smax value must be greater than or equal to the stock price.") 
        elif self._optionType == FinFixedStrikeLookbackOptionTypes.FIXED_PUT:
            smin = stockMinMax
            if smin > s0:
                raise FinError("The Smin value must be less than or equal to the stock price.")
        ...
        if self._optionType == FinFixedStrikeLookbackOptionTypes.FIXED_CALL:  
            ...
            if k > smax:
                ...
                if s0 == k:
                    ...
                    term = (-(s0/k)**(-w))*N(d1-2.0*b*sqrt(t)/v) + expbt*N(d1)
                ...
                if s0 == smax:
                    ...
                    term = (-(s0/smax)**(-w))*N(e1-2.0*b*sqrt(t)/v) + expbt*N(e1)

                v = df*(smax-k) + s0*dq*N(e1) - smax*df*N(e2) + s0*df*u*term
            ...
            if k >= smin:
                ...
                if s0 == smin:
                    ...
                    term = ((s0/smin)**(-w))*N(-f1+2.0*b*sqrt(t)/v) - expbt*N(-f1)

                v = df*(k-smin) - s0*dq*N(-f1) + smin*df*N(-f2) + s0*df*u*term
...
###############################################################################
    ...
    def valueMC(self, valueDate, stockPrice, interestRate, dividendYield, volatility, 
        ...
        model = FinGBMProcess()
        Sall = model.getPaths(numPaths,numTimeSteps,t,mu,stockPrice,volatility,seed)
                     
        # Due to antithetics we have doubled the number of paths        
        numPaths = 2 * numPaths
        payoff = np.zeros(numPaths)

        if optionType == FinFixedStrikeLookbackOptionTypes.FIXED_CALL:
            SMax = np.max(Sall,axis=1)
            smaxs = np.ones(numPaths)*smax
            payoff = np.maximum(SMax-k,0)
            payoff = np.maximum(payoff,smaxs-k)
        elif optionType == FinFixedStrikeLookbackOptionTypes.FIXED_PUT:

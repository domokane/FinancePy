...
class Tenor:
    '''
    A class to represent a Tenor such as '1D' or '10Y'. There is a unit (such as a day or a month)
    and a number representing how many units are in the tenor. For example 10Y means 10 years while 120m also means 10
    years

    We support limited conversion from bigger units to smaller units, which also uses pretty basic rules such as 
    1Y = 12M = 12*4 W = 12*4*7 D 
    so use conversion at your own risk. Specifically Y->M is ok and W->D is ok but Y->D is clearly not that great

    We also support limited arithmetic operations that make Tenors a linear space over integers (ie you can add tenors 
    ...
    def __add__(self, other):
        """Adds two tenors. Use the more fine-grained unit of the two for the result
    ...
    def convert_to_unit(self, new_units: TenorUnit):
        """Convert a tenor to an equivalent tenor in new units. Only "downcasting" is allowed ie W->D ok, D->W not ok
        This can obviously be improved. Very nominal conversations used, see the code for details
        ...
        if self._units.value < new_units.value:
            raise FinError(f"Cannot convert units from {self._units.name} to  {new_units.name}")
    ...
    def convert_to_same_units(cls, t1_in, t2_in):
        (t1, t2) = (t1_in, t2_in) if t1_in._units.value <= t2_in._units.value else (t2_in, t1_in)
        common_unit = t1._units
    ...
    def __eq__(self, other):
        """Equality in the strict sense, ie the units and the nummber of periods must be the same. In particular 12M != 1Y
        Perhaps this should be relaxed, but at the moment the idea is that a more relaxed equality could be explictly called 

    ...
    if scheme == FinHestonNumericalScheme.EULER.value:        
        ...
        for iPath in range(0,numPaths):
            ...
            for iStep in range(1,numSteps):
                ...
                x += mu*dt + K0 + (K1*vn + K2*vnp) + np.sqrt(K3*vn+K4*vnp)*zS
                sPaths[iPath,iStep] = exp(x)
                vn = vnp
    else:
...
class FinHestonModel():
    ...
    def __init__(self,v0,kappa,theta,sigma,rho):
        ...
        if 2.0*kappa*theta <= sigma and verbose == True:
...
################################################################################
    ...
    def value_Lewis(self,
        ...
        def phi(k_in,):
            ...
            Q = np.exp(-d*tau)
            T = T_m*(1.0-Q)/(1.0-g*Q) 
            W = kappa*theta*(tau*T_m-2.0*np.log((1.0-g*Q)/(1.0-g))/V)
            phi = np.exp(W + v0*T)
            return phi

        def phi_transform(x):
            integrand = lambda k: 2.0*np.real(np.exp(-1j*k*x) * phi(k))/(k**2+1.0/4.0)
...
################################################################################
    ...
    def value_Lewis_Rouah(self,
        ...
        def f(k_in):
            ...
            q=V*tau/2.0
            Q = np.exp(-e*q)
            H = np.exp((2.0*kappa*theta/V)*(q*g-np.log((1.0-h*Q)/(1.0-h)))+v0*g*(1.0-Q)/(1-h*Q))
    ...
    def value_Weber(self,
        ...
        def F(s,b):
            def integrand(u):
                ...
                A = kappa*((beta-d)*tau-2.0*np.log((1.0-g*Q)/(1.0-g)))/V
                v = np.exp(A*theta+B*v0 +1j*u*np.log(S0/(K*np.exp(-(r-q)*tau))))/(u*1j)
                return v.real

            area = 0.50 + (1.0/pi) * integrate.quad(integrand,0,np.inf)[0]
            return area

        v = S0 *exp(-q*tau)* F(1.0,kappa-rho*sigma) - exp(-r*tau) * K * F(-1.0,kappa)
        return(v)
    ...
    def value_Gatheral(self,
        ...
        def F(j):
            def integrand(u):
                ...
                C = kappa * (rminus*tau-(2.0/V)*np.log((1.0-R*Q)/(1.0-R)))  

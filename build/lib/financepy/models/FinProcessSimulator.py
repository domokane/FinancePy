...
class FinProcessSimulator():
    ...
    def getProcess(self, processType, t, modelParams, numAnnSteps,numPaths, seed):
        ...
        if processType == FinProcessTypes.GBM:
            ...
            paths = getHestonPaths(numPaths, numAnnSteps, t, drift,
                    stockPrice, v0, kappa, theta, sigma, rho, scheme.value, seed)
            return paths

        elif processType == FinProcessTypes.VASICEK:
            (r0,kappa,theta,sigma,scheme) = modelParams
            paths = getVasicekPaths(numPaths, numAnnSteps, t,
...
@njit(float64[:,:](int64,int64,float64,float64,float64,float64,float64,float64,float64,
    ...
    if scheme == FinHestonNumericalScheme.EULER.value:
        ...
        for iPath in range(0,numPaths):
            ...
            for iStep in range(1,numSteps+1):
                ...
                s += drift*s*dt + rtvplus*s*zS + 0.5*s*vplus*(zV*zV-dt)
...
@njit(float64[:,:](int64,int64,float64,float64,float64,float64,float64,int64,int64),
      cache=True,fastmath=True)
    ...
    if scheme == FinCIRNumericalScheme.EULER.value:
        ...
        for iPath in range(0,numPaths):
            ...
            for iStep in range(1,numSteps+1):
                ...
                r = r + kappa*(theta-rplus)*dt + sigmasqrtdt*z[iStep-1]*sqrtrplus
                ratePath[iPath,iStep] = r
        ...
        for iPath in range(0,numPaths):
            ...
            for iStep in range(1,numSteps+1):
                ...
                r = r + kappa*(theta-r)*dt + z[iStep-1]*sigmasqrtdt*sqrtrplus
        ...
        for iPath in range(0,numPaths):
            ...
            for iStep in range(1,numSteps+1):
                ...
                sqrtrplus = sqrt(max(r,0.0))
                c = 1.0 + (sigma*beta - 2.0*kappa*sqrtrplus)*dt/4.0/sqrtrplus

...
def zeroPrice(r0,a,b,sigma,t):
    ...
    A = (2.0*h*exp((a+h)*t/2.0)/denom)**(2.0*a*b/sigma/sigma)
    ...
    if scheme == FinCIRNumericalScheme.EULER.value:
        ...
        for iPath in range(0,numPaths):
            ...
            for iStep in range(1,numSteps):
                r = r + a*(b-r)*dt + z[iStep-1] * sigmasqrtdt * sqrt(max(r,0.0))
                ratePath[iStep] = r
        ...
        for iPath in range(0,numPaths):
            ...
            for iStep in range(1,numSteps):
                r = r + a*(b-r)*dt + z[iStep-1] * sigmasqrtdt * sqrt(max(0.0,r))
...
@njit(float64(float64,float64,float64,float64,float64,float64,int64,int64,int64))

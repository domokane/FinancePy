...
def indepLossDbnHeterogeneousAdjBinomial(numCredits,
    ...
    for iCredit in range(0,numCredits):
        ...
        vexact += lossRatioSquared * condDefaultProbs[iCredit] * (1.0 - condDefaultProbs[iCredit])
    ...
    term = diffAbove * diffAbove + (diffBelow * diffBelow - diffAbove * diffAbove) * diffAbove
    numer = vexact - term
    denom = vapprox - term
...
def indepLossDbnRecursionGCD(numCredits, 
    ...
    for iCredit in range(0,numCredits):
        ...
        for iLossUnit in range(loss,numLossUnits):
            nextDbn[iLossUnit] = prevDbn[iLossUnit - loss] * p + prevDbn[iLossUnit] * (1.0 - p)

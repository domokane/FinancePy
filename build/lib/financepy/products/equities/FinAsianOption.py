...
def valueMC_NUMBA(t0,t,tau, K,n, optionType,
    ...
    if t0 < 0: # we are in the averaging period
        ...
        if accruedAverage is None:
            raise FinError("If you are in the averaging period you need to enter the accrued average")
    ...
    for p in range(0,numPaths):
        ...
        s_1 = stockPrice * exp((mu - v2/2.0) * t0 + g[0] * sqrt(t0) * volatility )
        s_2 = stockPrice * exp((mu - v2/2.0) * t0 - g[0] * sqrt(t0) * volatility )
        ...
        for obs in range(0,n):
            ...
            s_1 = s_1 * exp((mu - v2/2.0) * dt + g[obs] * sqrt(dt) * volatility)
            s_2 = s_2 * exp((mu - v2/2.0) * dt - g[obs] * sqrt(dt) * volatility)
...
#    payoff_a = 0.0
    ...
    for ip in range(0,numPaths):
        s_1[ip] = stockPrice * exp((mu - v2/2.0) * t0 + g[ip] * sqrt(t0) * volatility )
        s_2[ip] = stockPrice * exp((mu - v2/2.0) * t0 - g[ip] * sqrt(t0) * volatility )
    ...
    for obs in range(0,n):
        ...
        for ip in range(0,numPaths):
            s_1[ip] = s_1[ip] * exp((mu - v2/2.0) * dt + g[ip] * sqrt(dt) * volatility)
            s_2[ip] = s_2[ip] * exp((mu - v2/2.0) * dt - g[ip] * sqrt(dt) * volatility)
...
###############################################################################
   ...
   def valueCurran(self,
        ...
        w = (1.0 - exp((2*b+sigma2)*h*n)) /  (1.0 - exp((2*b+sigma2)*h))
...
################################################################################
   ...
   def valueTurnbullWakeman(self,
        ...
        if b == 0:
            M1 = 1.0
            M2 = 2.0*exp(sigma2*t) - 2.0 * exp(sigma2*t0)*(1.0+sigma2*(t-t0))
            M2 = M2 / sigma2 / sigma2 / (t-t0) / (t-t0)
        else:
            M1 = S0 * (exp(b*t) - exp(b*t0))/(b*(t-t0))
            M2 = exp(a2*t)/a1/a2/(t-t0)/(t-t0) + (exp(a2*t0)/b/(t-t0)/(t-t0)) * (1.0/a2 - exp(b*(t-t0))/a1)
            M2 = 2.0 * M2 * S0 * S0
...
###############################################################################
   ...
   def valueMC(self,
        ...
        if valueDate > self._startAveragingDate and accruedAverage is None:
            raise FinError("In averaging period so need to enter accrued average.")

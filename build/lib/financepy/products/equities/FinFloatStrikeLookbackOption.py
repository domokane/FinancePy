...
class FinFloatStrikeLookbackOptionTypes(Enum):
    FLOATING_CALL=1
    FLOATING_PUT=2
...
################################################################################
    ...
    def value(self,
        ...
        if self._optionType == FinFloatStrikeLookbackOptionTypes.FLOATING_CALL:
            smin = stockMinMax
            if smin > s0:
                raise FinError("Smin must be less than or equal to the stock price.") 
        elif self._optionType == FinFloatStrikeLookbackOptionTypes.FLOATING_PUT:
            smax = stockMinMax
            if smax < s0:
        ...
        if self._optionType == FinFloatStrikeLookbackOptionTypes.FLOATING_CALL:
            ...
            if smin == s0:
                ...
                term = ((s0/smin)**(-w))*N(-a1+2.0*b*sqrt(t)/v) - expbt*N(-a1)
            ...
            if smax == s0:
                ...
                term = (-(s0/smax)**(-w)) * N(b1-2.0*b*sqrt(t)/v) + expbt*N(b1)
...
################################################################################
    ...
    def valueMC(self, valueDate, stockPrice, interestRate, dividendYield, volatility, 
        ...
        if optionType == FinFloatStrikeLookbackOptionTypes.FLOATING_CALL:
            SMin = np.min(Sall,axis=1)
            SMin = np.minimum(SMin,smin)
            payoff = np.maximum(Sall[:,-1]-SMin,0)
        elif optionType == FinFloatStrikeLookbackOptionTypes.FLOATING_PUT:
